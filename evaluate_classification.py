...
def main(args):
    ...
    for tot_class in total_clases:
        ...
        #lr_list = [0.1, 0.03, 0.01, 0.003,0.001, 0.0003, 0.0001, 0.00003, 0.00001, 0.000001]
        lr_all = []
        error_bars = {}
        for lr_search in range(1):
            ...
            for mem_size in [args.memory]:
                ...
                for lr in lr_list:
                    ...
                    for num, (name, param) in enumerate(maml.named_parameters()):
                        print('LOOOK AT ALL LAYERS!!', num, name)
                        param.learn = True

                    for num, (name, param) in enumerate(maml.named_parameters()):
                        # logger.info(name)
                        if name in frozen_layers:
                            print('LOOK AT FROZEN LAYERS!!', num, name)
                            param.learn = False
                ...
                # store error bars
                error_bar_index = int(mem_size) + int(lr_search)
                error_bars[error_bar_index] = results_mem_size
                
                error_bar_file = open("error_model_7_results.json", "w")
                json.dump(error_bars, error_bar_file)
                error_bar_file.close()
        ...
        for aoo in range(args.runs):
            ...
            a_file = open("meta_test_test_results.json", "w")
            json.dump(final_results_all, a_file)
            a_file.close()
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--schedule', type=int, nargs='+', default=[10,50,75,100,200,300,400,500,600], # [10,50,75,100,200,300,400,500,600]
