    ...
    def __init__(self, config, neuromodulation=True):
        ...
        for i, (name, param) in enumerate(self.config):
            if 'conv' in name:
              ...
              scaling_vec = nn.Parameter(torch.ones((1,1)))
              #torch.nn.init.kaiming_normal_(scaling_vec)
              self.vars.append(scaling_vec)
    ...
    def forward(self, x, vars=None, bn_training=True, feature=False):
        ...
        if self.Neuromodulation:
            ...
            for i in range(x.size(0)):
                ...
                #print('shape after first conv',nm_data.shape)
                ...
                #print('shape after first maxpooling',nm_data.shape)
                
                w,b = vars[4], vars[5]
                nm_data = conv2d(nm_data, w, b)
                #print('shape after second conv',nm_data.shape)
                ...
                nm_data = F.relu(nm_data)
                nm_data = maxpool(nm_data, kernel_size=2, stride=2)
                #print('shape after second maxpool',nm_data.shape)

                w,b = vars[8], vars[9]
                nm_data = conv2d(nm_data, w, b)
                #print('shape after third conv',nm_data.shape)
                ...
                #nm_data = conv2d(nm_data, w, b)
                #nm_data = F.relu(nm_data)
                #print('fourth layer size', nm_data.shape)

                #w,b = vars[8], vars[9]
                #nm_data = conv2d(nm_data, w, b)
                #nm_data = F.relu(nm_data)

                #print('final layer size', nm_data.shape)

                nm_data = nm_data.view(nm_data.size(0), 1008)

                # NM Output

                #print('size before sigmoid', nm_data.shape)
                
                w,b = vars[12], vars[13] # for scaling
                # print('size after fully connected layer of NM',F.linear(nm_data, w, b).shape)
                fc_mask = F.sigmoid(F.linear(nm_data, w, b)).view(nm_data.size(0), 2304)
                
                #scaling_vec, scaling_vec_bias  = vars[14], vars[15] # for scaling
                #fc_mask = fc_mask * scaling_vec + scaling_vec_bias
                #fc_mask = torch.exp(fc_mask)
                
                #print('fc mask size', fc_mask.shape)
                #print('gating vector!',fc_mask[0:100])
                ...
                #print('size after 1st convolution PR', data.shape)
                ...
                #print('size after 1st maxpool PR', data.shape)
                
                w,b = vars[18], vars[19]
                data = conv2d(data, w, b, stride=1)
                #print('size after 2nd convolution PR', data.shape)
                ...
                data = F.relu(data)
                data = maxpool(data, kernel_size=2, stride=2)
                #print('size after 2nd maxpool PR', data.shape)
                ...
                #print('size after 3rd convolution PR', data.shape)
                # modified - no gating applieds
                data = data*fc_mask


                w,b = vars[26], vars[27]
                data = F.linear(data, w, b)
                #print('very final size before pred', data.shape)
                try:
        ...
        #     #w,b = vars[14], vars[15]
        #     #data = F.linear(data, w, b)
        #     #print('very final size before pred code2', data[0][0][0][0:3])
        #     try:
        #         prediction = torch.cat([prediction, data], dim=0)
        #     except:
        #         prediction = data

            #print('size of prediction code2', data[0][0][0][0:3])
        ...
        ################ OML 1 ###################################
            ...
            for name, param in self.config:
                ...
                if name == 'conv2d':
                    ...
                    #print('var', idx, 'and', idx+1, 'layer', name, 'size', x.shape)
                    #print('number of index', idx, 'layer', name, 'size of output code 1', x[0][0][0][0:3])
                    idx += 2
                    #print('number of index', idx, 'size of output code 1', x.shape)
                    ...
                    x = F.linear(x, w, b)
                    #print('var', idx, 'and', idx+1, 'layer', name, 'size', x.shape)
                    #print('number of index', idx, 'layer', name, 'size of output code 1', x.shape)
                    if cat_var:
                        cat_list.append(x)
                    idx += 2
                    ...
                    #print('var', idx, 'and', idx+1, 'layer', name, 'bn_training', bn_training)
                    idx += 2
                    bn_idx += 2
                elif name == 'flatten':
                    # print(x.shape)
                    ...
                    #print('var', idx, 'and', idx+1, 'layer', name, 'kernel size', param[0], 'stride', param[1], 'padding', param[2])
