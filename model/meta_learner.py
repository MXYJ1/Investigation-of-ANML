...
class MetaLearingClassification(nn.Module):
    ...
    def inner_update(self, x, fast_weights, y, bn_training):
        ...
        #print('LOOK!!',self.net.parameters())
      
        #fast_weights_before = fast_weights
        grad = torch.autograd.grad(loss, fast_weights, allow_unused=False)
        ''''''''''''''''''''''''''''''''''''''''''''''''''
        #teddy = 0
        #weights_before = fast_weights[teddy][0]
        
        fast_weights = list(
            map(lambda p: p[1] - self.update_lr * p[0] if p[1].learn else p[1], zip(grad, fast_weights)))
        
        #weights_after = fast_weights[teddy][0]
        #weights_should_be = weights_before - self.update_lr * grad[teddy][0]
        #print('weights before', weights_before, 'weights after', weights_after, 'weights after should be', weights_should_be, 'grad', grad[teddy][0])
        # for i in range(22):
        #   if fast_weights.learn == True:
        #     print('layer', i, 'weights being learned')
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''
